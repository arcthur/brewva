...
- Workspace packages live under `packages/*`; primary surfaces include `runtime`, `search`, `substrate`, `provider-core`, `session-index`, `recall`, `deliberation`, `skill-broker`, `channels-telegram`, `ingress`, `tools`, `cli`, and `gateway`.
