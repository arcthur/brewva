...
describe("canonical hosted turn envelope RFC", () => {
  it("closes the envelope diagnostics durable projection decision", () => {
    ...
    const hostedTurnAdapterRfc = readRepoMarkdown(
    ...
    expect(hostedTurnAdapterRfc).toContain("Detailed recovery history stays process-local");
    expect(hostedTurnAdapterRfc).not.toContain("Decide whether detailed recovery history");
