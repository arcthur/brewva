...
describe("openai responses cache affinity headers", () => {
  test("adds cache affinity headers from rendered prompt cache keys", () => {
    const headers = buildOpenAIResponsesDefaultHeaders(
      TEST_MODEL as never,
      { messages: [] } as never,
      {
        promptCacheKey: "session-responses",
      } as never,
    );

    expect(headers.session_id).toBe("session-responses");
    expect(headers["x-client-request-id"]).toBe("session-responses");
  });

  test("preserves explicit header overrides and can suppress session_id", () => {
    const headers = buildOpenAIResponsesDefaultHeaders(
      TEST_MODEL as never,
      { messages: [] } as never,
      {
