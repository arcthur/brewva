...
describe("session supervisor watchdog bridge", () => {
  ...
  test("querySessionWire replays archived frames across multiple worker segments for the same public session id", async () => {
  ...
  test("supervisor does not synthesize child tape truth after worker hard exit", async () => {
  ...
  test("startup orphan sweep drops stale registry rows without writing child tape truth", async () => {
  ...
  test("startup orphan sweep ignores stale registry rows without an agent runtime writer", async () => {
