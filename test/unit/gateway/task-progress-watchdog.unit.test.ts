...
describe("task progress watchdog", () => {
  test("records watchdog blocker and detection event when task makes no semantic progress", () => {
    ...
    Date.now = () => now;
    try {
      const runtime = new BrewvaRuntime({ cwd: createTestWorkspace("watchdog-detect") });
  ...
  test("tracks no-spec exploration sessions instead of treating them as ineligible", async () => {
    ...
    Date.now = () => now;
    try {
      const runtime = new BrewvaRuntime({ cwd: createTestWorkspace("watchdog-no-spec") });
  ...
  test("when scan convergence blocker is active, emits event without writing watchdog blocker", () => {
    ...
    Date.now = () => now;
    try {
      const runtime = new BrewvaRuntime({ cwd: createTestWorkspace("watchdog-suppress") });
  ...
  test("when scan convergence arms after watchdog blocked the task, resolves the broader watchdog blocker", () => {
    ...
    Date.now = () => now;
    try {
      const runtime = new BrewvaRuntime({ cwd: createTestWorkspace("watchdog-superseded") });
  ...
  test("start schedules a single poller, stop clears it, and threshold policy sanitizes overrides", () => {
    ...
    Date.now = () => now;
    try {
      const runtime = new BrewvaRuntime({ cwd: createTestWorkspace("watchdog-lifecycle") });
