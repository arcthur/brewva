...
describe("prompt stability runtime state", () => {
  test("tracks prompt stability transitions and clears on session teardown", () => {
    ...
    const first = createOperatorRuntimePort(runtime).operator.context.prompt.observeStability(
      sessionId,
      {
    ...
    const unchanged = createOperatorRuntimePort(runtime).operator.context.prompt.observeStability(
      sessionId,
      {
        stablePrefixHash: "prefix-1",
        dynamicTailHash: "tail-1",
    ...
    const changedPrefix = createOperatorRuntimePort(
      runtime,
    ).operator.context.prompt.observeStability(sessionId, {
    ...
    const changedTail = createOperatorRuntimePort(runtime).operator.context.prompt.observeStability(
      sessionId,
      {
        stablePrefixHash: "prefix-2",
    ...
    const changedScope = createOperatorRuntimePort(
      runtime,
    ).operator.context.prompt.observeStability(sessionId, {
  ...
  test("tracks visible read epochs through the runtime context contract", () => {
    ...
    createOperatorRuntimePort(runtime).operator.context.visibleRead.rememberState(sessionId, state);
    expect(
    ...
    const nextEpoch = createOperatorRuntimePort(runtime).operator.context.visibleRead.advanceEpoch(
      sessionId,
      "history_pruned",
    );
