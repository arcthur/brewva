...
describe("tools review vocabulary", () => {
  test("keeps shared review vocabulary aligned with runtime skill vocabulary", () => {
    expect([...REVIEW_CHANGE_CATEGORIES] as readonly string[]).toEqual([
      ...RUNTIME_REVIEW_CHANGE_CATEGORIES,
    ]);
    expect([...REVIEW_LANE_NAMES] as readonly string[]).toEqual([...RUNTIME_REVIEW_LANE_NAMES]);
  });
});
