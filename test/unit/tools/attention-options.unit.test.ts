...
describe("trap-lens attention-option card source (Task 9)", () => {
  test("a session that touched the overbroad-tap fixture yields a trap_lens card whose body is the tap lens", async () => {
    const runtime = createRuntimeFixture();
    const sessionId = "attention-trap-lens-overbroad-1";
    writeFileSync(
    ...
    runtime.ops.tools.sourcePatch.plans.apply(sessionId, {
      ...
      planId: "plan-1",
      patchSetId: "patch-1",
      appliedPaths: ["OverbroadTap.swift"],
      failedPaths: [],
    });
    ...
    const result = await optionsTool.execute(
      "attention-options-trap-lens-1",
      { sources: ["trap_lens"], limit: 10 },
      new AbortController().signal,
      async () => undefined,
      toolContext(sessionId) as never,
    );
    ...
    const text = textContent(result as { content: Array<{ type: string; text?: string }> });
    expect(text).toContain("source=trap_lens");
    expect(text).toContain(TAP_LENS);
    expect(toolOutcomePayload(result)).toMatchObject({
      ok: true,
      options: [
        expect.objectContaining({
          sourceFamily: "trap_lens",
          // First-seen-wins across matchFileAgainstWriteVerifyTraps' write-
          // then-verify query order: event-tap-write-diff and event-tap-
          // verify-file share the identical lens text, so the write-phase
          // entry surfaces (deduped against, not alongside, the verify one).
          title: "event-tap-write-diff",
          whyRelevant: TAP_LENS,
          rootRef: "OverbroadTap.swift",
          authorityPosture: "none",
        }),
      ],
    });
  ...
  test("a session touching only non-tap files yields no trap-lens card", async () => {
    const runtime = createRuntimeFixture();
    const sessionId = "attention-trap-lens-none-1";
    writeFileSync(join(runtime.identity.cwd, "plain.ts"), "export const x = 1;\n");
    runtime.ops.tools.sourcePatch.plans.apply(sessionId, {
      ok: true,
      planId: "plan-1",
      patchSetId: "patch-1",
      appliedPaths: ["plain.ts"],
      failedPaths: [],
    });
    ...
    expect(toolOutcomePayload(result)).toMatchObject({ ok: true, options: [] });
  });

  test("a session with no applied patch sets at all yields no trap-lens card", async () => {
    const runtime = createRuntimeFixture();
    const sessionId = "attention-trap-lens-untouched-1";
  ...
  test("the leak fixture yields TWO distinct trap-lens cards (tap lens + passRetained lens), deterministic order", async () => {
    const runtime = createRuntimeFixture();
    const sessionId = "attention-trap-lens-leak-1";
    writeFileSync(
    ...
    const payload = toolOutcomePayload(result) as {
      options: Array<{ whyRelevant: string; title: string }>;
    };
    expect(payload.options.map((option) => option.whyRelevant)).toEqual([
      TAP_LENS,
      "balance every passRetained with a matching release, or use passUnretained when the callback does not take ownership",
    ]);

    // Repeated call is byte-identical in order — deterministic, not shuffled.
    const secondResult = await optionsTool.execute(
      "attention-options-trap-lens-4b",
      { sources: ["trap_lens"], limit: 10 },
      new AbortController().signal,
      async () => undefined,
      toolContext(sessionId) as never,
    );
    const secondPayload = toolOutcomePayload(secondResult) as {
      options: Array<{ whyRelevant: string }>;
    };
    expect(secondPayload.options.map((option) => option.whyRelevant)).toEqual(
      payload.options.map((option) => option.whyRelevant),
    );
  });

  test("the per-turn trap-lens cap is respected across many touched tap files", async () => {
    const runtime = createRuntimeFixture();
    const sessionId = "attention-trap-lens-cap-1";
    const overbroadContent = readFixture("overbroad-tap.swift");
    const appliedPaths: string[] = [];
    // 12 distinct touched tap files, each firing the same single tap lens —
    // the per-turn cap (8) must stop the source before all 12 are visited.
    for (let index = 0; index < 12; index += 1) {
      const path = `Tap${index}.swift`;
      writeFileSync(join(runtime.identity.cwd, path), overbroadContent);
    ...
    const result = await optionsTool.execute(
      "attention-options-trap-lens-5",
      { sources: ["trap_lens"], limit: 20 },
      new AbortController().signal,
      async () => undefined,
      toolContext(sessionId) as never,
    );

    const payload = toolOutcomePayload(result) as { options: readonly unknown[] };
    expect(payload.options.length).toBeLessThanOrEqual(8);
    expect(payload.options.length).toBeGreaterThan(0);
  });

  test("attention_consume re-reads the touched file and returns the trap id, file, and lens", async () => {
    const runtime = createBundledToolRuntime(createRuntimeFixture());
    const sessionId = "attention-trap-lens-consume-1";
    writeFileSync(
      join(runtime.identity.cwd, "OverbroadTap.swift"),
      readFixture("overbroad-tap.swift"),
    );
    runtime.capabilities.tools.sourcePatch.plans.apply(sessionId, {
      ok: true,
      planId: "plan-1",
      patchSetId: "patch-1",
      appliedPaths: ["OverbroadTap.swift"],
    ...
    const optionsResult = await optionsTool.execute(
      "attention-options-trap-lens-consume-1",
      { sources: ["trap_lens"], limit: 10 },
      new AbortController().signal,
      async () => undefined,
      toolContext(sessionId) as never,
    );
    const optionId = (toolOutcomePayload(optionsResult) as { options: Array<{ optionId: string }> })
      .options[0]?.optionId;
    if (!optionId) {
      throw new Error("expected_trap_lens_option_id");
    }
    expect(optionId).toBe("trap_lens:OverbroadTap.swift:event-tap-write-diff");

    const consumeResult = await consumeTool.execute(
      "attention-consume-trap-lens-1",
      { option_id: optionId, reason: "Confirm the tap lens applies." },
      new AbortController().signal,
      async () => undefined,
      toolContext(sessionId) as never,
    );
    ...
    expect(consumeText).toContain("event-tap-write-diff");
    expect(consumeText).toContain("OverbroadTap.swift");
    expect(consumeText).toContain(TAP_LENS);
  });
});
