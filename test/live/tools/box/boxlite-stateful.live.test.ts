...
import type { BoxPlane, BoxScope } from "@brewva/brewva-tools/contracts";
import { resolveConfiguredBoxPlane, type RuntimeBoxConfig } from "@brewva/brewva-tools/execution";
...
function createLiveBoxConfig(home: string): RuntimeBoxConfig {
  return {
    home,
    image: "alpine:latest",
    cpus: 1,
...
function createLivePlane(home: string): BoxPlane {
  return resolveConfiguredBoxPlane(undefined, createLiveBoxConfig(home));
}
...
describe("live: boxlite stateful box plane", () => {
  ...
  runLive("observes detached execution after launcher process exits", async () => {
    ...
    mkdirSync(workspace, { recursive: true });
    const launcherCode = `
import { resolveConfiguredBoxPlane } from "@brewva/brewva-tools/execution";

const config = JSON.parse(process.env.BREWVA_BOXLITE_LIVE_CONFIG);
const plane = resolveConfiguredBoxPlane(undefined, {
  home: config.home,
  image: "alpine:latest",
  cpus: 1,
  memoryMib: 512,
