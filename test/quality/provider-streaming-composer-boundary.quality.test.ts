...
const STREAM_DIR = "packages/brewva-provider-core/src/stream";
...
describe("provider streaming composer boundary", () => {
  ...
  test("stream module owns the composer primitives", () => {
    const allowedHosts = new Set([
      join(STREAM_DIR, "composer.ts"),
      join(STREAM_DIR, "run-provider-stream.ts"),
    ]);
    for (const file of listTypescriptFiles(STREAM_DIR)) {
      const content = readFileSync(join(repoRoot, file), "utf8");
      for (const construct of FORBIDDEN_CONSTRUCTORS) {
