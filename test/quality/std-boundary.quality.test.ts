...
describe("brewva std utility boundary", () => {
  test("std is an explicit-subpath leaf workspace package", () => {
    ...
    for (const subpath of [
      ...
      expect(exports[subpath], subpath).toBeDefined();
    }

    const dependencies = manifest["dependencies"] as Record<string, string>;
    expect(dependencies["@noble/hashes"]).toBe("2.2.0");
    expect(dependencies["p-limit"]).toBe("7.3.0");
    expect(dependencies["remeda"]).toBe("2.34.0");
    expect(dependencies["yaml"]).toBe("^2.8.3");
    expect(dependencies["ohash"]).toBeUndefined();
  });
  ...
  test("removed local utility files are not reintroduced", () => {
    for (const removedPath of [
      "packages/brewva-runtime/src/utils/hash.ts",
      "packages/brewva-runtime/src/utils/json.ts",
      "packages/brewva-runtime/src/utils/fs.ts",
      "packages/brewva-runtime/src/utils/markdown-frontmatter.ts",
      "packages/brewva-gateway/src/utils/deferred.ts",
      "packages/brewva-gateway/src/utils/json.ts",
      "packages/brewva-gateway/src/utils/serialized-async-task-runner.ts",
      "packages/brewva-gateway/src/cache/hash.ts",
      "packages/brewva-provider-core/src/utils/hash.ts",
      "packages/brewva-provider-core/src/utils/unknown-object.ts",
  ...
  test("old generic utility helper names are not reintroduced in production packages", () => {
    const offenders: string[] = [];
    const disallowedDeclarations =
      /\b(?:function|const)\s+(?:mapWithConcurrencyLimit|createSerializedAsyncTaskRunner|stableStringify|stableHash|shortHash)\b/gu;
    for (const file of listFiles("packages").filter((entry) => entry.endsWith(".ts"))) {
      if (file.startsWith("packages/brewva-std/src/")) continue;
      const matches = [...readRepoFile(file).matchAll(disallowedDeclarations)];
