...
describe("skill cascade orchestration", () => {
  test("accepted skill_selection proposals still arm a pending dispatch commitment", () => {
    const runtime = new BrewvaRuntime({
      cwd: createTestWorkspace("selection-commitment"),
  ...
  test("explicit cascade starts create runtime cascade intent", () => {
    const runtime = new BrewvaRuntime({
      cwd: createTestWorkspace("chain-commitment"),
  ...
  test("dispatch-disabled cascade source does not block explicit cascade starts", () => {
    const runtime = new BrewvaRuntime({
      cwd: createTestWorkspace("explicit-priority"),
  ...
  test("explicit intents still advance after manual completion", () => {
    const runtime = new BrewvaRuntime({
      cwd: createTestWorkspace("explicit-complete"),
      config: createConfig("auto"),
    });
  ...
  test("pause and resume keep the current cascade step stable", () => {
    const runtime = new BrewvaRuntime({
      cwd: createTestWorkspace("cascade-pause-resume"),
      config: createConfig("auto"),
    });
    const sessionId = "skill-cascade-pause-resume-1";

    const started = startExplicitChain(runtime, sessionId, ["repository-analysis", "review"]);
    expect(started.ok).toBe(true);
    expect(runtime.skills.getActive(sessionId)?.name).toBe("repository-analysis");

    const paused = runtime.skills.pauseCascade(sessionId, "operator_review");
    expect(paused.ok).toBe(true);
    expect(runtime.skills.getCascadeIntent(sessionId)?.status).toBe("paused");
    const pausedEvent = runtime.events.query(sessionId, {
      type: "skill_cascade_paused",
      last: 1,
    })[0];
    expect(pausedEvent?.payload).toMatchObject({
      reason: "operator_review",
      cursor: 0,
      nextSkill: "repository-analysis",
    });

    const resumed = runtime.skills.resumeCascade(sessionId, "operator_resume");
    expect(resumed.ok).toBe(true);
    expect(runtime.skills.getCascadeIntent(sessionId)?.status).toBe("running");
    expect(runtime.skills.getActive(sessionId)?.name).toBe("repository-analysis");
    const replannedEvent = runtime.events.query(sessionId, {
      type: "skill_cascade_replanned",
      last: 1,
    })[0];
    expect(replannedEvent?.payload).toMatchObject({
      reason: "operator_resume",
      strategy: "manual_resume",
    });
  });

  test("cancelled cascades do not resume progression when the active skill later completes", () => {
    const runtime = new BrewvaRuntime({
      cwd: createTestWorkspace("cascade-cancel-complete"),
      config: createConfig("auto"),
    });
    const sessionId = "skill-cascade-cancel-complete-1";

    const started = startExplicitChain(runtime, sessionId, ["repository-analysis", "review"]);
    expect(started.ok).toBe(true);
    expect(runtime.skills.getActive(sessionId)?.name).toBe("repository-analysis");

    const cancelled = runtime.skills.cancelCascade(sessionId, "manual_stop");
    expect(cancelled.ok).toBe(true);
    expect(runtime.skills.getCascadeIntent(sessionId)?.status).toBe("cancelled");

    const completed = runtime.skills.complete(
      sessionId,
      buildSkillOutputs(runtime, "repository-analysis"),
    );
    expect(completed.ok).toBe(true);
    expect(runtime.skills.getCascadeIntent(sessionId)?.status).toBe("cancelled");
    expect(runtime.skills.getCascadeIntent(sessionId)?.cursor).toBe(0);
    expect(runtime.skills.getActive(sessionId)).toBeUndefined();
    expect(runtime.skills.getOutputs(sessionId, "repository-analysis")).toBeDefined();
    expect(runtime.events.query(sessionId, { type: "skill_cascade_finished" })).toHaveLength(0);
  });

  test("clearing session state drops cascade intent and prevents later control operations", () => {
    const runtime = new BrewvaRuntime({
      cwd: createTestWorkspace("cascade-clear-state"),
      config: createConfig("auto"),
    });
    const sessionId = "skill-cascade-clear-state-1";

    const started = startExplicitChain(runtime, sessionId, ["repository-analysis", "review"]);
    expect(started.ok).toBe(true);
    expect(runtime.skills.getCascadeIntent(sessionId)).toBeDefined();

    runtime.session.clearState(sessionId);

    expect(runtime.skills.getCascadeIntent(sessionId)).toBeUndefined();
    expect(runtime.skills.getActive(sessionId)).toBeUndefined();
    const resumed = runtime.skills.resumeCascade(sessionId, "after_clear");
    expect(resumed.ok).toBe(false);
    if (!resumed.ok) {
      expect(resumed.reason).toBe("intent_not_found");
    }
  });
});
