...
describe("OpenTUI import quarantine", () => {
  ...
  test("keeps tui OpenTUI imports behind its internal runtime", () => {
    const allowlist = new Set(["packages/brewva-tui/runtime/internal-opentui-runtime.ts"]);
    const violations = walk(resolve(repoRoot, "packages", "brewva-tui"))
      .map((file) => ({
        file: relative(repoRoot, file),
        specifiers: directOpenTuiImportSpecifiers(file),
      }))
