...
describe("deliberation proposal record helpers", () => {
  ...
  test("listInjectableContextPacketRecords breaks same-timestamp packet conflicts deterministically by proposal id", () => {
    const sessionId = uniqueSessionId("records-context-same-timestamp");
    const sameTimestampRuntime = {
      proposals: {
        list() {
          return [
            {
              proposal: {
                id: `${sessionId}:context_packet:alpha`,
                kind: "context_packet",
                issuer: "test.operator",
                subject: "alpha packet",
                payload: {
                  label: "OperatorMemo",
              ...
              receipt: {
                proposalId: `${sessionId}:context_packet:alpha`,
                decision: "accept",
                policyBasis: ["context_packet"],
                reasons: ["context_packet_committed"],
                committedEffects: [],
            ...
            {
              proposal: {
                id: `${sessionId}:context_packet:omega`,
                kind: "context_packet",
                issuer: "test.operator",
                subject: "omega packet",
                payload: {
                  label: "OperatorMemo",
                  content: "Omega summary",
                  packetKey: "summary",
                },
                evidenceRefs: buildEvidence(sessionId),
                createdAt: 400,
              ...
              receipt: {
                proposalId: `${sessionId}:context_packet:omega`,
                decision: "accept",
                policyBasis: ["context_packet"],
                reasons: ["context_packet_committed"],
                committedEffects: [],
    ...
    const effective = listInjectableContextPacketRecords(sameTimestampRuntime, sessionId, {
      now: 1_000,
    });
    expect(effective).toHaveLength(1);
    expect(effective[0]?.proposal.id).toBe(`${sessionId}:context_packet:omega`);
    expect(effective[0]?.proposal.payload.content).toBe("Omega summary");
  });
});
