...
import { planHeartbeatWake, writeCognitionArtifact } from "@brewva/brewva-deliberation";
import { createTestWorkspace } from "../helpers/workspace.js";

describe("deliberation proactivity engine", () => {
  test("skips if_signal wake-ups when only a foreign session signal exists", async () => {
    const workspace = createTestWorkspace("deliberation-proactivity-engine-foreign-signal");
  ...
  test("uses same-session summary signals to wake continuation work", async () => {
    const workspace = createTestWorkspace("deliberation-proactivity-engine-same-session");
