...
- `tape_handoff` / `tape.handoff` events in the session event tape:
  ...
  references
- `.brewva/session-index/*.sqlite` (plus `-wal`/`-shm` siblings): rebuildable
  SQLite + FTS5 query plane for typed recall and insights queries; safe to delete
  and rebuild from event tape. Non-writer processes read the live WAL directly,
  so there is no separate read-snapshot copy
...
drawing conclusions. Treat session-index rows as indexed evidence
