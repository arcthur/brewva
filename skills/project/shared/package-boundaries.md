...
| `@brewva/brewva-substrate`         | contract root plus explicit session, prompt, resources, tools, host-api, provider, compaction, context-budget, and agent-protocol subpaths (provenance/execution/persistence are internal; no turn/sdk subpath) | replay authority, credentials, hosted policy                                                                  |
...
| `@brewva/brewva-acp-adapter`       | ACP protocol-to-gateway session adapter, stdio ACP agent lifecycle, request shape normalization, and gateway session wire projection into ACP updates                                                           | runtime kernel semantics, hosted session execution, generic channel transports, or gateway daemon supervision |
...
- substrate mechanisms are imported through explicit domain subpaths:
  ...
  `/agent-protocol`. persistence/provenance/execution are substrate-internal —
  recovered from the public surface (WS5: zero external consumers, consumed via
  relative paths). `/sdk` and `/turn` are not exported: direct hosts construct
...
- substrate provenance (source-info vocabulary) and execution (small primitives;
  event buses keep subscriber access separate from controller emit authority)
  are substrate-internal mechanism domains, consumed via relative paths. They are
  not public subpaths and not exported from the substrate root — re-publish only
  if a real second consumer appears (WS5 recovered them as zero-consumer seams)
