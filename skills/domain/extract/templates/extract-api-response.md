...
```json
{
  "plans": [{ "name": "Starter", "price": "9", "currency": "USD" }]
}
```
...
```json
{
  ...
  "zca_contract": {
    "schema_summary": ["plans[].name:string", "plans[].price:number", "plans[].currency:string"],
    "invariants": ["price > 0", "currency must be an uppercase ISO 4217 code"],
    "repair_budget": 2,
    "size_policy": "inline_summary"
  },
  "zca_result": {
