...
- Decision: One shell, multiple operator layers. The default experience is a single-session conversation shell. Operator surfaces are overlays, pagers, or sibling footer surfaces, not separate terminal modes with their own truth.
- Date: `2026-04-14`
- Status: accepted
- Stable docs:
...
- Code anchors:
  ...
  - `packages/brewva-cli/src/shell/domain/task-overlay-preview.ts`
...
- One shell, multiple operator layers. The default experience is a single-session conversation shell. Operator surfaces are overlays, pagers, or sibling footer surfaces, not separate terminal modes with their own truth.
...
- Task and inspect review stay replay-visible. Task browser summaries, subagent footer detail, and inspect pager drill-down consume the same delegation and workflow state as the rest of Brewva; they do not operate a TUI-private task registry.
