...
- `recall_search` is the on-demand read surface. The model decides when to call it and whether to preserve results in the workbench or durable knowledge capture.
- Recall stays source-typed and scope-bounded. Tape evidence, workbench-admitted memory, knowledge capture, promotion drafts, and repository precedent remain distinguishable and repository-root scoped by default.
