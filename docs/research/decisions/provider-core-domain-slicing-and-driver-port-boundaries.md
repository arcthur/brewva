...
- Code anchors:
  ...
  - `packages/brewva-substrate/src/agent-protocol/types.ts`
...
- Provider events have one canonical contract in provider-core. The runtime turn loop consumes that contract through gateway/provider adapters instead of redeclaring provider event families, so advisory fields such as `parseStatus` cannot drift across package boundaries.
