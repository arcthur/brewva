...
- Audit, skill selection: already subtracted to deterministic reasons (the fuzzy-matching engine removal); receipts (`skill.selection.recorded`) plus the then-named adoption projection were the evidence that graded it (fuzzy offers dominated surfacing with roughly 1-in-8 subsequent SKILL.md opens). That projection is now named `opened` because a temporal read join proves neither compliance nor effectiveness. The account-then-grade-then-calibrate loop this ran is the pattern this decision generalizes.
