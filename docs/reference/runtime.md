...
Operational mutation is available only through repo-owned ports such as the
hosted runtime port and operator runtime port.
...
`config` is a deep-readonly snapshot after normalization. The raw runtime root
does not expose `operator` or `extensions`.
...
or `runtime.operator`, and they do not grant tool authority.
...
Runtime surface member count: 83. Authority: 29. Inspect: 40. Operator: 14.
...
through `createHostedRuntimePort(...)`; operator products receive them through
`createOperatorRuntimePort(...)`; managed tools do not receive them.
...
Hosted extensions are repo-owned ports exposed by `createHostedRuntimePort(...)`
and tool-scoped ports exposed by `createToolRuntimePort(...)`. They expose only
allowlisted methods; they do not spread service instances, leak private state,
or preserve old implementation paths. The raw runtime root does not expose
`extensions`.
