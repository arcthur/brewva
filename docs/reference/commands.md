...
For Google, `/model` is the primary first-class auth surface: it can import an
existing official Gemini CLI credential from `~/.gemini/oauth_creds.json` or the
official encrypted file fallback at `~/.gemini/gemini-credentials.json`.
It also exposes the Google Gemini OAuth browser flow when the host process has
`BREWVA_GOOGLE_OAUTH_CLIENT_ID` and `BREWVA_GOOGLE_OAUTH_CLIENT_SECRET`.
Both paths store a refreshable hosted OAuth credential and require a Google
Cloud project ID from the prompt, `GOOGLE_CLOUD_PROJECT`, or
