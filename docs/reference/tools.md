...
Goal tools are control-plane tools. `get_goal` reads the active session goal;
`update_goal` only accepts `complete` or `blocked`. Both are dynamically
exposed only when the current session goal is active and capability selection
allows the required `capabilities.goal.*` paths. Paused, cleared,
budget-limited, complete, and blocked goals hide these tools and fail closed.
...
- Memory, recall, attention options, precedent, solution records, knowledge,
  workbench note/evict operations, and iteration facts:
