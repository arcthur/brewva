 ...
 * names WHERE it came from (source). `ref` is the evidence's stable id (finding
 * id, outcome ref, or deterministic entry ref); `verdict` is present for the
 * kinds that carry pass/fail (`authored` coverage carries no verdict — it only
...
export interface FitnessProjection {
  ...
  /**
   * Ids of `must`-modality atoms on a HIGH-RISK class (runtime/security) whose
   * state is NOT `satisfied` (i.e. `unverified` or `likelySatisfied`): NO
   * deterministic OR independent pass bears on them, so the only support is an
   * author self-claim or nothing at all. An independent read is still owed — the
   * perspective the authoring stream cannot mint on its own work (authorship
   * taints verification). First-appearance order. A `violated` atom is excluded:
   * a live fail is a discrepancy, a different signal, not an unmet independence
   * need.
   *
   * This is the model-facing independence signal the delegation surface reads.
   * It is descriptive and drives no gate (axiom 18); the model decides whether to
   * discharge it by delegating a fresh-context review.
   */
 ...
 * A `pass` here is strong positive evidence (reaches `satisfied`); a `fail` is
 * treated as a violation, same as a deterministic fail's downgrade but graded
 * `advisory_conflict` (it is not deterministic evidence).
 */
...
/**
 * The requirement classes that owe an independent read — the failure-mode
 * classes (`runtime`, `security`) where an author self-claim is the weakest
 * possible basis and a fresh-context review is the perspective the authoring
 * stream cannot mint on its own work. This is the single source of "high risk"
 * for {@link FitnessProjection.independenceDebtAtoms}. Classes absent here
 * (`ux`/`packaging`/`architecture`) and unclassified atoms owe no independence
 * debt: self-review clears those honestly, so demanding independence there would
 * be noise.
 */
const HIGH_RISK_REQUIREMENT_CLASSES: ReadonlySet<RequirementRiskClass> = new Set([
  "runtime",
  "security",
]);

function isHighRiskAtom(atom: RequirementAtom): boolean {
  return atom.riskClass !== undefined && HIGH_RISK_REQUIREMENT_CLASSES.has(atom.riskClass);
}
...
interface AtomAccumulator {
  ...
  /** A deterministic pass keyed to the atom — reaches `satisfied`. */
  hasDeterministicPass: boolean;
  /** An independent pass that names the atom — reaches `satisfied`. */
...
export function projectRequirementFitness(input: RequirementFitnessInput): FitnessProjection {
  ...
  for (const authored of input.authoredOutcomes) {
    for (const atomId of authored.atomRefs) {
      ...
      // Author coverage is a self-claim: it caps at `likelySatisfied` and never
      // proves satisfaction (the perspective axis governs it).
  ...
  // High-risk `must` atoms owe an independent read until `satisfied`: only a
  // deterministic OR independent pass reaches `satisfied`, so an atom stuck at
  // `unverified`/`likelySatisfied` has none — the authoring stream cannot mint
  // the independent perspective on its own work (authorship taints verification).
  // Non-high-risk atoms are excluded: self-review clears those honestly, so
  // demanding independence there would be noise. A `violated` atom is a live fail
  // (a discrepancy), a different signal, so it is excluded too.
  ...
  for (const entry of atoms) {
    ...
    if (
      ...
      !isHighRiskAtom(accumulator.atom)
    ) {
      continue;
    }
    if (entry.state === "unverified" || entry.state === "likelySatisfied") {
      independenceDebtAtoms.push(entry.atomId);
    }
  }
