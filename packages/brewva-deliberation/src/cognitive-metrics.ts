...
export type CognitiveMetricToolSurface = "base" | "skill" | "operator" | "external";

export interface CognitiveMetricsOptions {
  resolveToolSurface?: (toolName: string) => CognitiveMetricToolSurface | undefined;
}
...
function isProductiveTool(
  ...
  return resolveToolSurface?.(normalized) !== "operator";
}
...
function recordProductiveToolMetrics(
  ...
  if (!toolName || payload.verdict !== "pass" || !isProductiveTool(toolName, resolveToolSurface)) {
    return;
  }
  const currentTurn = getCurrentRuntimeTurn(sessionId);
  const turnIndex = currentTurn + 1;
  const toolSurface = resolveToolSurface?.(toolName) ?? "external";
...
function processRecordedToolResult(
  ...
  recordProductiveToolMetrics(runtime, sessionId, state, payload, resolveToolSurface);
}
...
export function createCognitiveMetricsLifecycle(
  runtime: BrewvaRuntime,
  options: CognitiveMetricsOptions = {},
): CognitiveMetricsLifecycle {
