import type { ClassifiedError } from "./state.js";
...
export function classifyRecoveryError(error: unknown): ClassifiedError {
  const detail = normalizeRuntimeError(error);
  if (looksLikeMaxOutputError(error)) {
    return {
      family: "max_output",
      reason: "provider_response_exhausted",
      detail,
    };
  }
  if (looksLikeRetryableProviderError(error)) {
    return {
      family: "provider_retryable",
