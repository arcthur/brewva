...
import { resolveNextThreadLoopDecision } from "./decision.js";
import {
...
import type { RecoveryDecision } from "./state.js";
import {
...
async function resolveFailureRecovery(input: {
  ...
  for (;;) {
    ...
    if (
      ...
      failureDecision.recovery.kind === "compact" &&
      input.loop.runtime &&
    ...
    if (
      failureDecision.action === "revert_then_stream" &&
      failureDecision.recovery.kind === "reasoning-revert" &&
      input.loop.runtime &&
      pendingRevert
    ) {
    ...
    const recoveryDecision: RecoveryDecision = failureDecision.recovery;
    ...
    if (recoveryResult.outcome === "recovered") {
      ...
      state = withDecision(
        withRecoveryHistory(state, {
          policy: failureDecision.policy,
          outcome: recoveryDecision.kind === "compact" ? "recovered" : "recovered",
        }),
        "complete",
      );
      return {
