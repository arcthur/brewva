...
import {
  ...
  SCHEDULER_PAUSE_PARSE_OPTIONS,
  SCHEDULER_RESUME_PARSE_OPTIONS,
  START_PARSE_OPTIONS,
  ...
  parseOptionalPathFlag,
  printGatewayCliValueError,
  pushStringFlag,
  resolveManagedToolModeFlag,
} from "./cli/parse.js";
import {
...
function resolveGatewayAdminCommand(argv: string[]): GatewayAdminCommand {
  ...
  switch (command) {
    ...
    case "rotate-token":
    case "logs":
      return { kind: command, argv: rest };
    default:
      return { kind: "unknown", command, argv: rest };
  }
}
...
export async function runGatewayCliOperation(
  ...
  const command = resolveGatewayAdminCommand(argv);
  const handlers = {
    start: handleStart,
    ...
    status: handleStatus,
    stop: handleStop,
    "scheduler-pause": handleSchedulerPause,
    "scheduler-resume": handleSchedulerResume,
    "heartbeat-reload": handleHeartbeatReload,
    "rotate-token": handleRotateToken,
    logs: handleLogs,
  } satisfies Record<
    Exclude<GatewayAdminCommand["kind"], "help" | "unknown">,
  ...
  if (command.kind === "unknown") {
    ...
    console.error(`Error: unknown gateway command "${command.command}".`);
    printGatewayHelp();
    return {
      handled: true,
      exitCode: 1,
  ...
  return {
    handled: true,
    exitCode: await handlers[command.kind](command.argv),
  };
}
