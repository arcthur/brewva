...
export const SESSION_REWIND_CHECKPOINT_SCHEMA = "brewva.session.rewind.checkpoint.v1";
export const SESSION_REWIND_SCHEMA = "brewva.session.rewind.v1";
export const SESSION_REDO_SCHEMA = "brewva.session.rewind.redo.v1";
export const SESSION_SUPERSEDE_SCHEMA = "brewva.session.rewind.superseded.v1";
...
function readSuccessfulRewindPayload(
  ...
  const divergenceNote = readDivergenceNote(payload.divergenceNote);
  ...
  return {
    ...
    abandonedCheckpointIds: readStringArray(payload.abandonedCheckpointIds),
...
function readDivergenceNote(value: unknown): SessionRewindDivergenceNote | undefined {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return undefined;
  }
  const record = value as Record<string, unknown>;
  const kind =
    record.kind === "workspace_ahead" || record.kind === "conversation_ahead"
      ? record.kind
      : undefined;
  const text = normalizeOptionalString(record.text);
  const patchSetCount =
    typeof record.patchSetCount === "number" && Number.isFinite(record.patchSetCount)
      ? Math.max(0, Math.floor(record.patchSetCount))
      : undefined;
  if (!kind || !text || patchSetCount === undefined) {
    return undefined;
  }
  return {
    kind,
    text,
    patchSetCount,
    parentLeafEntryId: normalizeNullableString(record.parentLeafEntryId),
  };
}
