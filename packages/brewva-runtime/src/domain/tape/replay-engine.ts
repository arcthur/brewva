...
function cloneToolFailureEntry(entry: ReplayToolFailureEntry): ReplayToolFailureEntry {
  return {
    ...entry,
    args: cloneJsonValue(entry.args),
  };
}
...
function resolveFailureClassFromPayload(payload: ToolResultRecordedEventPayload): FailureClassKey {
  const rawFailureContext = payload.failureContext;
  ...
  const args = rawFailureContext?.args ?? {};
  const outputText = rawFailureContext?.outputText ?? "";
...
function parseFailureContext(
  payload: ToolResultRecordedEventPayload,
  fallbackTurn: number,
  timestamp: number,
  anchorEpoch: number,
  ...
  if (!raw) return null;
  const toolName = payload.toolName.trim() || "unknown_tool";
  const args = raw.args;
  const outputText = raw.outputText;
...
function reduceEvidenceState(
  ...
  const next: ReplayEvidenceState = {
    ...
    failureRecords: state.failureRecords + (payload.verdict === "fail" ? 1 : 0),
  ...
  if (payload.verdict === "fail") {
    ...
    const failure = parseFailureContext(payload, eventTurn, timestamp, next.anchorEpoch);
...
function applyEventToView(
  ...
  } else if (event.type === TOOL_RESULT_RECORDED_EVENT_TYPE) {
