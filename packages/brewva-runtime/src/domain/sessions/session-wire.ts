...
import type { SessionWireAttemptReason, SessionWireFrame, SessionWireSource } from "./wire.js";
import { SESSION_WIRE_SCHEMA } from "./wire.js";
...
function buildTurnTransitionFrames(
  ...
  const attemptReason = readSessionWireAttemptReason(payload.reason);
  ...
  frames.push({
    ...
    attempt: payload.attempt ?? transitionAttempt,
...
function compileDurableEvent(
  ...
  switch (event.type) {
    ...
    case TURN_RENDER_COMMITTED_EVENT_TYPE: {
      const payload = readTurnRenderCommittedEventPayload(event);
      if (!payload) {
        return [];
      }
      if (typeof event.turn === "number" && Number.isFinite(event.turn)) {
