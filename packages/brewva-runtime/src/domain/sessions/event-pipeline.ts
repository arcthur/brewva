...
const DEBUG_EVENT_TYPES = BREWVA_SESSION_LOCAL_EVENT_TYPE_SET;
...
const TYPED_EVENT_DESCRIPTOR_BY_TYPE = new Map<
  string,
  (typeof BREWVA_TYPED_EVENT_DESCRIPTORS)[number]
>(BREWVA_TYPED_EVENT_DESCRIPTORS.map((descriptor) => [descriptor.type, descriptor]));
...
function assertRecordedEventInput<TPayload extends object>(
  input: RuntimeRecordEventInput<TPayload>,
): void {
  const descriptor = TYPED_EVENT_DESCRIPTOR_BY_TYPE.get(input.type);
  if (!descriptor) {
    return;
  }
  if (!readBrewvaEventPayload(input, descriptor)) {
    throw new Error(`invalid_recorded_event_payload:${input.type}`);
  }
}
...
export class EventPipelineService {
  ...
  private appendEvent<TPayload extends object>(
    ...
    assertRecordedEventInput(input);
