...
import type { SkillSystemInstallResult } from "./types.js";
...
function looksLikeSkillsRoot(path: string): boolean {
  return (
    isDirectory(join(path, "system")) ||
    isDirectory(join(path, "project")) ||
    isDirectory(join(path, "core")) ||
    isDirectory(join(path, "domain")) ||
    isDirectory(join(path, "operator")) ||
    isDirectory(join(path, "meta")) ||
    isDirectory(join(path, "internal"))
  );
}

function resolveCheckoutSkillsDir(modulePath: string): string | undefined {
...
function resolveBundledSkillsSourceDir(input: EnsureBundledSystemSkillsInput): string {
  ...
  const checkoutSkillsDir = resolveCheckoutSkillsDir(modulePath);
  if (checkoutSkillsDir) {
    return checkoutSkillsDir;
  }

  throw new Error(
    `[skills_system_install] bundled skills payload not found. Checked ${execSkillsDir} and repository checkout ancestors from ${modulePath}.`,
  );
}
