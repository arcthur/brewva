...
import type { BrewvaIntentId, BrewvaWalId } from "../../core/identifiers.js";
import { asBrewvaIntentId } from "../../core/identifiers.js";
import {
...
import { SCHEDULE_EVENT_TYPE } from "./events.js";
import {
...
export class SchedulerService {
  ...
  createIntent(
    ...
    if (!parentSessionId) {
      return { ok: false, reason: "missing_parent_session_id" };
    }
    ...
    if (!hasRunAt && !hasCron) {
      return { ok: false, reason: "missing_schedule_target" };
    }
    if (hasRunAt && hasCron) {
      return { ok: false, reason: "runAt_and_cron_are_mutually_exclusive" };
    }
    if (hasRunAt && normalizedTimeZoneInput) {
      return { ok: false, reason: "timeZone_requires_cron" };
    }
    const now = this.now();
    ...
    if (this.intentsById.has(intentId)) {
      return { ok: false, reason: "intent_id_already_exists" };
    }
    ...
    if (hasCron) {
      ...
      if (normalizedNextRunAt === undefined) {
        return { ok: false, reason: "cron_has_no_future_match" };
      }
    } else {
      if (typeof input.runAt !== "number" || !Number.isFinite(input.runAt) || input.runAt <= now) {
        return { ok: false, reason: "invalid_runAt" };
      }
    ...
    if (activeIntents.length >= this.runtimePort.scheduleConfig.maxActiveIntentsGlobal) {
      return { ok: false, reason: "max_active_intents_global_exceeded" };
    }
    ...
    if (activeInSession >= this.runtimePort.scheduleConfig.maxActiveIntentsPerSession) {
      return { ok: false, reason: "max_active_intents_per_session_exceeded" };
    }
    ...
    if (!appended) {
      return { ok: false, reason: "events_store_disabled" };
    }

    const intentKey = asBrewvaIntentId(intentId);
    const intent = this.intentsById.get(intentKey);
    if (!intent) return { ok: false, reason: "intent_persist_failed" };
    if (this.enableExecution) {
  ...
  cancelIntent(
    ...
    const parentSessionId = normalizeOptionalString(input.parentSessionId);
    if (!parentSessionId) return { ok: false, reason: "missing_parent_session_id" };
    const intentId = normalizeOptionalString(input.intentId);
    ...
    if (intent.parentSessionId !== parentSessionId)
      return { ok: false, reason: "intent_owner_mismatch" };
    if (intent.status !== "active") return { ok: false, reason: "intent_not_active" };
    ...
    const appended = this.appendScheduleEvent(payload);
    if (!appended) return { ok: false, reason: "events_store_disabled" };
  ...
  updateIntent(
    ...
    if (intent.status !== "active" && !canReactivate)
      return { ok: false, reason: "intent_not_active" };
    ...
    if (!updated) return { ok: false, reason: "intent_persist_failed" };
    if (this.enableExecution) {
