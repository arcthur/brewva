...
function buildRecoveredWorkerResult(
  ...
  const workerId = payload?.runId;
  if (!workerId) {
    return undefined;
  }
  const kind = payload.kind;
  const manifestRef = payload.artifactRefs?.find((ref) => ref.kind === "patch_manifest")?.path;
  ...
  if (event.type === SUBAGENT_COMPLETED_EVENT_TYPE) {
    ...
    if (patches) {
      return {
        workerId,
        status: "ok",
        summary: payload.summary ?? "Recovered delegated patch outcome.",
        patches,
      };
    }
    return {
  ...
  return {
    ...
    status: "error",
    summary:
      payload.summary ?? payload.error ?? payload.reason ?? "Recovered delegated patch failure.",
    errorMessage: payload.error ?? payload.reason ?? "Recovered delegated patch failure.",
  };
}
