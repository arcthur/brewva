import type { BrewvaToolCallId, BrewvaWalId } from "../../core/identifiers.js";
import { asBrewvaToolCallId } from "../../core/identifiers.js";
import { readToolLifecycleEventPayload } from "../../events/descriptors.js";
import {
...
import { buildTurnEnvelope } from "../channels/api.js";
...
function readObservedToolLifecycle(event: {
  type: string;
  payload?: unknown;
}): { toolCallId: BrewvaToolCallId; toolName: string } | null {
  const payload = readToolLifecycleEventPayload(
    event as Pick<BrewvaEventRecord, "type" | "payload">,
  );
  if (!payload) {
    return null;
  }
  return {
    toolCallId: asBrewvaToolCallId(payload.toolCallId),
...
export class ToolLifecycleRecoveryWalService {
  ...
  private handleToolExecutionStart(event: {
    ...
    const payload = readObservedToolLifecycle(event);
    if (!payload) {
      return;
    }
