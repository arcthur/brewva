import { readTaskStuckDetectedEventPayload } from "../../events/descriptors.js";
import {
...
export const TASK_WATCHDOG_TURN_LIFECYCLE_PLACEMENT = {
  ...
  source: "packages/brewva-runtime/src/domain/task/task-watchdog.ts",
...
export class TaskWatchdogService {
  ...
  pollTaskProgress(input: PollTaskProgressInput): void {
    ...
    const latestPayload = latestDetected ? readTaskStuckDetectedEventPayload(latestDetected) : null;
    if (
      latestPayload &&
      latestDetected &&
  ...
  private maybeClearTaskProgressStall(sessionId: string): void {
    ...
    const detectedPayload = latestDetected
      ? readTaskStuckDetectedEventPayload(latestDetected)
      : null;
