...
import type { TaskSpec, TaskTargetDescriptor } from "./types.js";
...
export function resolveTaskTargetDescriptor(input: {
  cwd: string;
  workspaceRoot: string;
  spec?: TaskSpec;
}): TaskTargetDescriptor {
  const roots = resolveTaskTargetRoots(input);
  return {
    primaryRoot: roots[0] ?? resolve(input.cwd),
    roots,
  };
}
