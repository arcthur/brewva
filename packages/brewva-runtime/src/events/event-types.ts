...
export type BrewvaEventDurabilityClass = "authority-bearing" | "audit-only" | "session-local";

export const BREWVA_AUTHORITY_BEARING_EVENT_TYPES = [
  TAPE_ANCHOR_EVENT_TYPE,
  TAPE_CHECKPOINT_EVENT_TYPE,
  REASONING_CHECKPOINT_EVENT_TYPE,
  REASONING_REVERT_EVENT_TYPE,
...
const BREWVA_AUTHORITY_BEARING_EVENT_TYPE_SET: ReadonlySet<BrewvaRegisteredEventType> = new Set(
  BREWVA_AUTHORITY_BEARING_EVENT_TYPES,
);

export const BREWVA_AUDIT_ONLY_EVENT_TYPES = BREWVA_REGISTERED_EVENT_TYPES.filter(
  (type) => !BREWVA_AUTHORITY_BEARING_EVENT_TYPE_SET.has(type),
);
...
export const BREWVA_EVENT_DURABILITY_BY_TYPE: Readonly<Record<string, BrewvaEventDurabilityClass>> =
  Object.freeze(
    Object.fromEntries([
      ...BREWVA_AUTHORITY_BEARING_EVENT_TYPES.map((type) => [type, "authority-bearing" as const]),
      ...BREWVA_AUDIT_ONLY_EVENT_TYPES.map((type) => [type, "audit-only" as const]),
      ...BREWVA_SESSION_LOCAL_EVENT_TYPES.map((type) => [type, "session-local" as const]),
    ]),
  );
