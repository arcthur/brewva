...
export const REDO_EVENT_TYPE = "redo" as const;
...
export const TURN_GOVERNANCE_DECISION_EVENT_TYPE = "turn_governance_decision" as const;
...
export const CORRECTION_CHECKPOINT_RECORDED_EVENT_TYPE = "correction_checkpoint_recorded" as const;
export const CORRECTION_UNDO_COMPLETED_EVENT_TYPE = "correction_undo_completed" as const;
...
export const REVERSIBLE_MUTATION_REDONE_EVENT_TYPE = "reversible_mutation_redone" as const;
...
export const BREWVA_SOURCE_OF_TRUTH_EVENT_TYPES = [
  ...
  GOVERNANCE_COMPACTION_INTEGRITY_FAILED_EVENT_TYPE,
  GOVERNANCE_COMPACTION_INTEGRITY_ERROR_EVENT_TYPE,
] as const satisfies readonly BrewvaRegisteredEventType[];

export const BREWVA_SESSION_LOCAL_EVENT_TYPES = ["tool_parallel_read"] as const;
...
const BREWVA_SOURCE_OF_TRUTH_EVENT_TYPE_SET: ReadonlySet<BrewvaRegisteredEventType> = new Set(
  BREWVA_SOURCE_OF_TRUTH_EVENT_TYPES,
);

const BREWVA_REBUILDABLE_SIGNAL_EVENT_TYPE_SET: ReadonlySet<BrewvaRegisteredEventType> = new Set(
  BREWVA_REBUILDABLE_SIGNAL_EVENT_TYPES,
);

export const BREWVA_DURABLE_EVIDENCE_EVENT_TYPES = BREWVA_REGISTERED_EVENT_TYPES.filter(
  (type) =>
    !BREWVA_SOURCE_OF_TRUTH_EVENT_TYPE_SET.has(type) &&
    !BREWVA_REBUILDABLE_SIGNAL_EVENT_TYPE_SET.has(type) &&
    !(BREWVA_SESSION_LOCAL_EVENT_TYPES as readonly string[]).includes(type),
);
...
export const BREWVA_EVENT_DURABILITY_BY_TYPE: Readonly<Record<string, BrewvaEventDurabilityClass>> =
  Object.freeze(
    Object.fromEntries([
      ...BREWVA_SOURCE_OF_TRUTH_EVENT_TYPES.map((type) => [type, "source_of_truth" as const]),
      ...BREWVA_DURABLE_EVIDENCE_EVENT_TYPES.map((type) => [type, "durable_evidence" as const]),
      ...BREWVA_REBUILDABLE_SIGNAL_EVENT_TYPES.map((type) => [type, "rebuildable_signal" as const]),
      ...BREWVA_SESSION_LOCAL_EVENT_TYPES.map((type) => [type, "session_local" as const]),
    ]),
  );
...
export function getBrewvaEventDurabilityClass(
  ...
  return BREWVA_EVENT_DURABILITY_BY_TYPE[type];
}
