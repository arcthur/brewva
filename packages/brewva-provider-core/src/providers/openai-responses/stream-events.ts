...
export function processResponsesStream<TApi extends Api>(
  ...
  return BrewvaEffect.gen(function* () {
    ...
    yield* runAsyncIterableEffect(openaiStream, (event) =>
      BrewvaEffect.gen(function* () {
        if (event.type === "response.created") {
          ...
          if (item.type === "reasoning") {
            ...
            output.content.push(currentBlock);
            yield* stream.push({ type: "text_start", contentIndex: blockIndex(), partial: output });
          } else if (item.type === "function_call") {
            const toolItemId = item.id ?? item.call_id;
            yield* toolCalls.begin(
              toolItemId,
              {
          ...
          yield* toolCalls.appendArgumentsDelta(event.item_id, event.delta);
        } else if (event.type === "response.function_call_arguments.done") {
          yield* toolCalls.replaceArguments(event.item_id, event.arguments);
          ...
          if (item.type === "reasoning" && currentBlock?.type === "thinking") {
            ...
            const toolItemId = item.id ?? item.call_id;
            yield* toolCalls.replaceArguments(toolItemId, item.arguments || "{}");
            yield* toolCalls.finalize(toolItemId, {
              id: `${item.call_id}|${item.id}`,
              name: item.name,
            });
          }
          ...
          return yield* failProviderStream(msg);
        }
      }),
    );
  });
