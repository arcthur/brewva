...
export function canAppendToolRuntimeGuardedSupplementalBlocks(
  ...
  const toolsExtension = runtime?.extensions?.tools;
  if (!toolsExtension) return false;
  return (
    typeof Reflect.getOwnPropertyDescriptor(toolsExtension, "appendGuardedSupplementalBlocks")
      ?.value === "function"
  );
}
