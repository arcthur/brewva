...
import type { SourceDocument, SourceGraphEdge } from "./ir.js";
import {
  renderCycles,
  renderDigest,
...
export function createSourceIntelligenceTools(options?: {
  ...
  const codeOutline = codeOutlineFactory.define({
    ...
    async execute(_id, params, _signal, _onUpdate, ctx) {
      ...
      try {
        document = await engine.loadDocument(absolutePath);
      } catch (error) {
  ...
  const codeDigest = codeDigestFactory.define({
    ...
    parameters: Type.Object({
      ...
      max_tokens: Type.Optional(
        Type.Integer({ minimum: 100, maximum: MAX_DIGEST_BUDGET, default: DEFAULT_DIGEST_BUDGET }),
      ),
      limit: Type.Optional(
    ...
    async execute(_id, params, _signal, _onUpdate, ctx) {
      ...
      try {
        graph = await engine.buildGraph(roots);
      } catch (error) {
        return failTextResult(sourceIntelligenceError(error));
      }
      ...
      recordSourceIntelligenceEvent(codeDigestFactory.runtime, sessionId, {
        ...
        totalFiles: graph.documents.length,
        budgetTokens: budget,
        durationMs: Date.now() - startedAt,
      });
      ...
      let renderedDigest = renderDigest({
        ...
        graph,
        omittedFiles: selected.omittedFiles,
        omittedDeclarations,
      });
      ...
      while (digestDocuments.length > 1 && renderedTokens > budget) {
        ...
        renderedDigest = renderDigest({
          ...
          documents: digestDocuments,
          graph,
          omittedFiles: Math.max(0, graph.documents.length - digestDocuments.length),
          omittedDeclarations,
        });
      ...
      return textResult(renderedDigest, {
        ...
        omitted: {
          files: Math.max(0, graph.documents.length - digestDocuments.length),
  ...
  const codeSurface = codeSurfaceFactory.define({
    ...
    async execute(_id, params, _signal, _onUpdate, ctx) {
      ...
      try {
        surface = await engine.resolveSurface(absolutePath);
      } catch (error) {
        return failTextResult(sourceIntelligenceError(error));
      }
      const sessionId = getToolSessionId(ctx);
