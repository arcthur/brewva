  ...
  return define(
    {
      ...
      promptGuidelines: [
        ...
        "Reach for a delegated run when the task needs cross-3+-file investigation, diagnosis, a second-opinion review pass, or parallel slice analysis; a delegated sweep returns a bounded outcome instead of raw tool frames, protecting the parent window and the compaction budget.",
        ...
        "Launch independent runs in a single assistant message with multiple tool calls; keep objectives specific and pass only the context references the child needs, avoiding broad parent-context dumps.",
